 ...
 */
public class LogTracingUtilsOld {
    ...
    @Getter
    @Setter
    private static Set<AttributeKeys> contextBounded = new HashSet<>();

    private LogTracingUtils() {
    }

    public static LogTracingUtils LogTracingUtils(){
    ...
    public enum AttributeKeys {
        ...
        EVENT_OUTCOME("event.outcome", "{eventOutcome-not-found}", Set.of("success","fail")),
    ...
    private static Map.Entry<String, String> serializeDetailsToMdcMap(Map<String, ?> details){
        String rawDetails = "{}";
        if (details != null) {
            try {
        ...
        return Map.entry(CTX_DETAILS_KEY,rawDetails);
    }
    ...
    public static void withErrorMdc(
                                    Throwable error,
                                    Map<AttributeKeys, ?> attributes,
                                    Map<String, ?> details,
                                    Runnable block
    ) {
        Map<String, Object> mdcMap = new HashMap<>();

        Map.Entry<String,String> detailEntry = serializeDetailsToMdcMap(details);

        mdcMap.put(detailEntry.getKey(),detailEntry.getValue());

        mdcMap.put(
                AttributeKeys.ERROR_TYPE.getKey(),
                error != null
                        ? error.getClass().getName()
                        : AttributeKeys.ERROR_TYPE.getDefaultValue()
        );
        mdcMap.put(
                AttributeKeys.ERROR_MESSAGE.getKey(),
                error != null && error.getMessage() != null
                        ? error.getMessage()
                        : AttributeKeys.ERROR_MESSAGE.getDefaultValue()
        );
    ...
    public static void withErrorMdc(
        ...
        withErrorMdc(error, null, null, block);
    }
    ...
    public static void withContextDetailsMdc(
        ...
        withContextDetailsMdc( null, details, block);
    }
